...
\begin{enumerate}
    ...
    \item Installer les dépendances JavaScript : \texttt{yarn install} ou \texttt{npm install}
    ...
    \item Compiler les assets : \texttt{yarn encore dev}
    \item Configurer les clés Stripe dans \texttt{.env} : \\
          \texttt{STRIPE\_PUBLIC\_KEY=pk\_test\_...} \\
          \texttt{STRIPE\_SECRET\_KEY=sk\_test\_...}
    \item Lancer le serveur : \texttt{symfony server:start} ou, sur Windows, exécuter le script \texttt{.\textbackslash run-tests-and-start.bat} pour lancer les tests et démarrer l'application.
\end{enumerate}
